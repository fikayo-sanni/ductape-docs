...
The file must be a top-level JSON array. Do not wrap it in
`{ "notifications": [], "messages": [] }`. Each notification may include its own nested
`messages` array.
...
[
      ...
      },
      {
        ...
        "emails": {
          ...
          "smtp": {
            ...
            "sender_email": "no-reply@example.com",
            "auth": {
              "user": "$Secret{postmark-user}",
              "pass": "$Secret{postmark-password}"
            },
            "secure": true
          }
        }
  ...
  },
  {
    ...
    "messages": [
      {
        "tag": "push-alerts:match-launch",
        "name": "Match launch",
        "push_notification": {
          "title": "Match ready",
          "body": "{{matchName}} is ready",
          "data": {}
        }
      }
    ]
...
Notification component tags cannot contain `:` (for example, `push-alerts`). Message tags use
`component-tag:message-tag` (for example, `push-alerts:match-launch`).

For SMTP, `sender_email` is required and `secure` must be a JSON boolean; `secure` cannot be a
`$Secret{...}` placeholder. Firebase cloud-linked environments should use a GCP connection with
the `notifications` scope. Creation records the link but does not prove Google-side authorization;
runtime delivery requires the FCM API and an appropriately authorized service account.
