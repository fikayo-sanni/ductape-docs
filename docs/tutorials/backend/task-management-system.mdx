...
Learn how to build a comprehensive task management system with scheduled jobs, file storage, team collaboration, quotas, notifications, and automated features using Ductape SDK.
...
```bash
npm install @ductape/sdk@latest express multer dotenv
...
async function createTask(data: {
  ...
  // Start task feature
  await startTaskFeature(task.rows[0]);
...
async function startTaskFeature(task: any) {
...
export const taskLifecycleFeature = {
  name: 'task-lifecycle',
  version: '1.0.0',
  steps: [
