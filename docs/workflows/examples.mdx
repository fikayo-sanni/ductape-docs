...
Real-world workflow patterns using the code-first API. For conditionals, loops, and switch (e.g. `branchOverrides`, `recordInput`, `recordScenarios`), see [Building Workflows - Conditionals, loops, and switch](./building-workflows#conditionals-loops-and-switch).

## E-Commerce Order Fulfillment

A complete order processing workflow with payment, inventory, and notifications. Because the handler returns early when validation fails, use **branchOverrides** so the compiler records the payment and later steps; at runtime those steps run only when the condition holds.
