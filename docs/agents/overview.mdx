...
|-----------|--------|-------------|
| Actions | `ctx.api.run()` | Call external APIs |
...
| Publishing | `ctx.messaging.produce()` | Message broker publishing |
...
- [Features](../features/overview) - Combine agents with features
