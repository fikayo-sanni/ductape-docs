...
`ductape_generate_payload` and `ductape_generate_snippet` require a `publishable_key` argument, identical to `ductape_execute`. The key is forwarded as the `x-access-key` header to `POST /integrations/v1/payloads/generate` and validated by the backend.
...
All three tools authenticate with a `publishable_key`. Treat it as a secret:
