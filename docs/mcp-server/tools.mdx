...
The Ductape MCP server exposes runtime, schema, documentation, CLI, Events-discovery, migration,
and portable-function setup tools. Responses are JSON text in MCP content blocks. Errors return
`isError: true` with a message string.

## `ductape_features_validate_project`

Run this read-only tool before every code-first Feature synchronization. It parses TypeScript and
JavaScript Feature definitions under `ductape/features/` and reports file, line, column, Feature tag,
...
It rejects native branches and loops, array control-flow helpers, host-bound time/random/environment
access, uninspectable dynamic definitions, and legacy recording options without an explicit legacy
mode. A failed result means the agent must stop; `features sync` must not be called. The CLI repeats
the same validation before starting application registration code.

```text
ductape_features_validate_project({ project_dir })
  -> fix every diagnostic
  -> ductape_features_validate_project({ project_dir })
  -> ductape_cli({ command: "features sync", project_dir })
```

---

## `ductape_function_setup`

Use this whenever a Feature calls application-owned code through `ctx.functions`. It returns the
well-known HTTPS URLs, framework-specific raw-body adapter requirements, HMAC headers, validation
checklist, and fail-closed conditions.
...
The MCP agent should register local handlers and attempt secure remote exposure in application
code. It must not claim remote availability until the route is reachable and rejects an invalid
signature. See [Portable application functions](/features/portable-functions).
...
| Compact product inventory | `products components list --product-tag my-product --json` |
| Inspect one component type | `products components get --product-tag my-product --type healthchecks --json` |
| List apps | `apps list` |
...
| Manage health checks | `resources health list` / `resources health create -f healthcheck.json` |
...
:::info Administrative configuration
Use `products environments` for product environments and `resources quota`, `resources fallback`, `resources jobs`, or `resources health` for those administrative resources. App actions and auths remain [Workbench](https://cloud.ductape.app)-managed. Features are defined in application code and registered at runtime.
:::
...
| Configure product environments | `ductape_cli` (`products environments ...`) |
| Configure quotas, fallbacks, jobs, or health checks | `ductape_cli` (`resources <type> ...`) |
| Configure app actions or auths | Workbench UI |
| Define features | Application code (`features.define`) |
| Validate Feature source before sync | `ductape_features_validate_project` |
| Persist validated Feature definitions | `ductape_cli` (`features sync`) |
