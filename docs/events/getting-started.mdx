...
Call `events.produce()` with the event string and the message payload:
...
Call `events.consume()` with the event string and a callback. The callback receives each message as it arrives:
...
For consuming, wire a service method with `@Events.Consume` (or use the SDK `events.consume()` in your module's `onModuleInit`):
