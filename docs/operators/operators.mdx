...
| **Now** | `$Now` or `$Now()` | Current timestamp (ms) at workflow execution | `$Now` |

These references (and all operators below) are **resolved at runtime** when a workflow or feature runs—e.g. `$Now` becomes the execution-time timestamp, and `$Input{...}` is filled from the run input.
