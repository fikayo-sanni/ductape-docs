...
Operators in Ductape are built-in data transformation functions that enable you to manipulate, format, and combine data within your features. They are a core part of Ductape's **declarative data referencing system**, allowing you to transform data without writing custom code.
...
1. **Platform Independence** - Your features are defined as pure data structures (JSON), not executable code. This makes them:
...
2. **Security** - No arbitrary code execution means:
   ...
   - Safe to store user-defined features
...
Ductape provides multiple ways to reference data within your features:
...
| **Now** | `$Now` or `$Now()` | Current timestamp (ms) at feature execution | `$Now` |

These references (and all operators below) are **resolved at runtime** when a feature or feature runs—e.g. `$Now` becomes the execution-time timestamp, and `$Input{...}` is filled from the run input.
...
### Example 5: Using Secrets and App Auth in Features
...
- Features can be stored, versioned, and migrated easily
