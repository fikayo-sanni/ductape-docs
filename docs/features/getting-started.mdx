...
:::important
Feature handlers are compiled into portable JSON. Every `ctx.step()` callback must record a Ductape
primitive or a [portable function](./portable-functions). Ductape does not serialize arbitrary
JavaScript closures and never uses a recording-time return value as runtime output.
:::
