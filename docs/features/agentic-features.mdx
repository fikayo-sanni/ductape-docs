...
# Agentic Features (RFC)

:::caution
This is a **Request for Comments (RFC)** document describing a proposed feature. Agentic Features are not yet implemented in Ductape. This document outlines the technical design and API for community feedback.
:::

Agentic Features extend Ductape's feature engine to support AI-driven, autonomous multi-step processes where a Large Language Model (LLM) dynamically decides the next action based on observations and reasoning.

---

## Overview

Traditional Ductape features are **deterministic** - steps execute in a predefined order. Agentic features are **dynamic** - an LLM observes the current state, reasons about what to do next, and selects from available tools until a goal is achieved.
...
## When to Use Agentic Features

Use **Agentic Features** when you need:
    ...
    },
    {
      ...
      handler: async (ctx, params) => {
        await ctx.messaging.produce({
          event: 'support-queue:escalation',
          message: {
...
Each iteration of an agentic feature follows this pattern:
...
```typescript
{
  ...
  handler: async (ctx, params) => {
    ...
    // Process refund
    const refund = await ctx.api.run({
      app: 'stripe',
      event: 'create-refund',
...
### Approval Feature Integration

```typescript
// External approval via feature signal
...
Always cite your sources and distinguish between facts and analysis.`,
  ...
  tools: [
    ...
    },
    {
      ...
      handler: async (ctx, params) => {
        // Generate report
        const report = await ctx.api.run({
          app: 'document-api',
          event: 'generate',
          input: { body: params },
        });
...
- [Features](./overview) - Traditional deterministic features
