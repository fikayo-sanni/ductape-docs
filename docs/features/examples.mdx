...
Real-world feature patterns using the code-first API. For new conditional and iteration code, follow [Building Features - Portable control flow](./building-workflows#portable-control-flow).

:::warning Legacy examples
Some longer examples on this page demonstrate `branchOverrides` or native JavaScript control flow from the earlier recording API. Treat those examples as migration references. New Features must use `ctx.branch`/`ctx.when`, `ctx.each(ctx.sampleInput...)`, portable Functions for runtime-sized algorithms, and `ctx.transform` for runtime-generated values.
:::
...
await ductape.features.define({
  ...
  handler: async (ctx) => {
    ...
    const verificationCode = ctx.transform.uuid();
