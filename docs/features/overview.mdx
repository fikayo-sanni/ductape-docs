...
3. **Context (ctx)** - Provides access to Ductape components and feature state
...
Stored Features execute their compiled step graph; the handler is not rerun at runtime. Use `ctx.branch` with `ctx.when` for runtime decisions, `ctx.each` with `ctx.sampleInput` for fixed graph expansion, and portable Functions for runtime-sized collection algorithms. Do not apply native `if/else`, `??`, loops, or array methods to unresolved `ctx.input` or step-result proxies.

Validate the entire project before syncing. Validation is read-only, and sync runs the same fail-closed preflight before any registration code starts:
...
See [Building Features - Portable control flow](./building-workflows#portable-control-flow).

## Next Steps

- [Getting Started](./getting-started) - Set up your first feature
- [Building Features](./building-workflows) - Definition options and portable control flow
