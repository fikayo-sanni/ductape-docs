...
:::warning Portable operations only
...
serialized and now fails compilation. See [Portable functions](./portable-functions).
:::
...
**Deprecated.** Prefer [ctx.messaging.produce()](#ctxmessaging) above.
...
- `ctx.vector.query({ vector, values, topK, namespace?, filter?, includeValues?, includeMetadata?, minScore? })`
- `ctx.vector.upsert({ vector, vectors, namespace?, wait? })`
- `ctx.vector.upsertOne({ vector, id, values, metadata?, namespace? })`
- `ctx.vector.deleteVectors({ vector, ids?, namespace?, deleteAll?, filter? })`
- `ctx.vector.execute({ vector, action, input })` for a registered vector action
...
`ctx.session` and `ctx.sessions` serve different purposes. `ctx.session` is the read-only token inherited by the current Feature execution. `ctx.sessions` records explicit lifecycle operations against a configured product session.

```ts
const issued = await ctx.step('start-player-session', () =>
  ctx.sessions.start({
    session: 'player-session',
    data: { playerId: ctx.input.playerId },
  }),
)

await ctx.step('revoke-old-session', () =>
  ctx.sessions.revoke({
    session: 'player-session',
    sessionId: ctx.input.oldSessionId,
  }),
)
```

Available methods are `start`, `verify`, `refresh`, `revoke`, and `list`. The executor supplies the Feature product and environment. Starting or refreshing a session returns tokens but does not mutate the immutable `ctx.session` inherited by the current execution; pass a returned token explicitly when a later application operation needs it.
