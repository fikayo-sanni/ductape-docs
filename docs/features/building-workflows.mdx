...
The following example documents the older `branchOverrides` mechanism for existing Features. It requires `controlFlowMode: 'legacy'`. Do not generate new code in this style. New Features should use [`ctx.branch` and `ctx.when`](#runtime-branches); the legacy form remains here only to help migrate stored definitions safely.
:::
