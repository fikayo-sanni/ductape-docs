---
sidebar_position: 3
sidebar_label: Building Features
---
...
| Option | Purpose |
|--------|--------|
| **branchOverrides** | Legacy compatibility for handlers written before `ctx.branch`. Do not use it in new Feature code. |
| **recordInput** | Sample input exposed as `ctx.sampleInput` during recording so loops run and record steps. It never replaces runtime `ctx.input` references. Use unique step tags per iteration. |
| **recordScenarios** | Legacy compile-time graph discovery for several sample inputs. Prefer `ctx.branch` for runtime decisions. |
...
| You want to… | Use |
|--------------|-----|
| Runtime `if/else` | `ctx.branch(ctx.when.*, { then, else })` |
| Combine conditions | `ctx.when.and(...)` or `ctx.when.or(...)` |
| Expand a fixed sample collection | `recordInput` + `ctx.each(ctx.sampleInput.items, ...)` |
| Process a runtime-sized collection | A registered portable Function invoked through `ctx.functions` |
| Generate runtime strings or identifiers | `ctx.transform.concat`, `now`, `uuid`, and the string transforms |
...
await ductape.features.define({
  ...
  handler: async (ctx) => {
    const enabled = ctx.sampleInput.regions.filter(Boolean);
    await ctx.each(enabled, async (region, index) => {
...
Native `map`, `filter`, `find`, and indexing are allowed on concrete `ctx.sampleInput` values for compile-time graph discovery. They are not runtime collection primitives. Use a portable Function for runtime-sized `map`, `filter`, `find`, `some`, `every`, `reduce`, or arbitrary loops.

:::danger Never generate these patterns
```typescript
...
:::warning Migration reference only
The following example documents the older `branchOverrides` mechanism for existing Features. Do not generate new code in this style. New Features should use [`ctx.branch` and `ctx.when`](#runtime-branches); the legacy form remains here only to help migrate stored definitions safely.
:::
